...
const ReceiptVoucherReport: React.FC = () => {
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `Receipt_Voucher_Report_${selectedFinYear || "All"}.csv`;
    a.click();
              ...
              {monthlySummaries.map((mSummary) => {
                return (
                  <tr
                    ...
                    onClick={() => navigate(`/app/reports/voucher/receipt/detail/${mSummary.monthName}`)}
                  ...
                  >
                    <td className="px-6 py-3.5 whitespace-nowrap text-sm text-gray-900 flex items-center justify-between">
                      <span className="font-medium text-green-700 group-hover:underline flex items-center gap-1.5">
                        {mSummary.monthName}
                      </span>
