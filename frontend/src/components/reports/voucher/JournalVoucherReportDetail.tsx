...
import { generateJournalXmlContent } from "../../voucherRegister/journalXmlGenerator";
...
const JournalVoucherReportDetail: React.FC = () => {
  ...
  const deleteHandler = async (id: string) => {
    ...
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "Do you really want to delete this journal voucher?",
      icon: "warning",
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/vouchers/${id}?ownerType=${ownerType}&ownerId=${ownerId}&voucherType=journal`,
        { method: "DELETE" }
      );
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `Journal_Register_${monthData.monthName}_${monthData.year}.csv`;
    a.click();
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Journal Voucher Register - ${monthData.monthName} ${monthData.year}</title>
          <style>
        ...
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-3">
              ...
              Journal Register - {monthData.monthName} {yearDisplay}
            </h1>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-gray-200">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Displaying all journal vouchers recorded in {monthData.monthName}.
            </p>
          </div>
                    ...
                    <tr key={v.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-3.5 whitespace-nowrap text-sm font-medium text-center">
                        <div className="flex items-center justify-center space-x-3">
                          <button
                            onClick={() => navigate(`/app/vouchers/journal/edit/${v.id}`)}
          ...
          {filteredVouchers.length === 0 && (
            <div className="text-center py-10 bg-gray-50">
              <p className="text-gray-500 text-base">
                No journal vouchers found for {monthData.monthName} {yearDisplay}.
              </p>
            </div>
          )}
        </div>
...
export default JournalVoucherReportDetail;
