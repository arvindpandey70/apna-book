...
import { generatePurchaseXmlContent } from "../../voucherRegister/purchaseVoucherRegister/purchaseXmlGenerator";
...
const PurchaseVoucherReportDetail: React.FC = () => {
  ...
  const getParticulars = (voucher: VoucherEntry): string => {
    ...
    if (voucher.partyName) return voucher.partyName;
    if (voucher.partyId) return getLedgerName(voucher.partyId);
    return "Purchase A/c";
  };

  const getVoucherStatus = (voucher: VoucherEntry): string => {
    if (!voucher.referenceNo) return "draft";
    const totalAmount = Number(voucher.total || voucher.billAmount || voucher.netAmount || 0);
  ...
  const deleteHandler = async (id: string) => {
    ...
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "Do you really want to delete this purchase voucher?",
      icon: "warning",
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: "Purchase voucher has been deleted successfully.",
      });
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `Purchase_Register_${monthData.monthName}_${monthData.year}.csv`;
    a.click();
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Purchase Voucher Register - ${monthData.monthName} ${monthData.year}</title>
          <style>
        ...
        <body>
          <div class="header">
            <h2>Purchase Voucher Register</h2>
        ...
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-3">
              ...
              Purchase Register - {monthData.monthName} {yearDisplay}
            </h1>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-gray-200">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Displaying all purchase vouchers recorded in {monthData.monthName}.
            </p>
          </div>
                ...
                {filteredVouchers.map((v) => {
                  ...
                  return (
                    <tr key={v.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-3.5 whitespace-nowrap text-sm text-gray-500">
                        {formatDate(v.supplier_invoice_date || v.supplierInvoiceDate || v.date)}
                      </td>
                      <td
                      ...
                      <td className="px-6 py-3.5 whitespace-nowrap text-sm font-medium text-center">
                        <div className="flex items-center justify-center space-x-3">
                          <button
                            onClick={() => navigate(`/app/vouchers/purchase/edit/${v.id}`)}
          ...
          {filteredVouchers.length === 0 && (
            <div className="text-center py-10 bg-gray-50">
              <p className="text-gray-500 text-base">
                No purchase vouchers found for {monthData.monthName} {yearDisplay}.
              </p>
            </div>
          )}
        </div>
...
export default PurchaseVoucherReportDetail;
