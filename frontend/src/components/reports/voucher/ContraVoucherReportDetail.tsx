...
import { generateContraXmlContent } from "../../voucherRegister/contraXmlGenerator";
...
const ContraVoucherReportDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(
      `${import.meta.env.VITE_API_URL}/api/vouchers?companyId=${companyId}&ownerType=${ownerType}&ownerId=${ownerId}&voucherType=contra`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  const deleteHandler = async (id: string) => {
    ...
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "Do you really want to delete this contra voucher?",
      icon: "warning",
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/vouchers/${id}?ownerType=${ownerType}&ownerId=${ownerId}&voucherType=contra`,
        { method: "DELETE" }
      );
      ...
      Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: "Contra voucher has been deleted successfully.",
      });
  ...
  const handleGenerateXml = async (voucher: VoucherEntry) => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/vouchers/${voucher.id}?companyId=${companyId}&ownerType=${ownerType}&ownerId=${ownerId}&voucherType=contra`
      );
      ...
      const xmlContent = generateContraXmlContent(fullVoucher, companyName, dbLedgers);

      Swal.close();
      setPreviewXml({
        content: xmlContent,
        filename: `ContraVoucher_${voucher.number || voucher.id}.xml`,
      });
    } catch (error) {
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `Contra_Register_${monthData.monthName}_${monthData.year}.csv`;
    a.click();
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Contra Voucher Register - ${monthData.monthName} ${monthData.year}</title>
          <style>
    ...
    <div className="pt-[56px] px-4 min-h-screen pb-12">
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center">
          <button
            onClick={() => navigate("/app/reports/voucher/contra")}
            className="p-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 transition-colors mr-3"
            title="Back to Contra Report"
          >
            <ArrowLeft size={22} />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-3">
              <ArrowLeftRight className="text-purple-600" size={28} />
              Contra Register - {monthData.monthName} {yearDisplay}
            </h1>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-gray-200">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Displaying all contra vouchers recorded in {monthData.monthName}.
            </p>
          </div>
                    ...
                    <tr key={v.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-3.5 whitespace-nowrap text-sm font-medium text-center">
                        <div className="flex items-center justify-center space-x-3">
                          <button
                            onClick={() => navigate(`/app/vouchers/contra/edit/${v.id}`)}
          ...
          {filteredVouchers.length === 0 && (
            <div className="text-center py-10 bg-gray-50">
              <p className="text-gray-500 text-base">
                No contra vouchers found for {monthData.monthName} {yearDisplay}.
              </p>
            </div>
          )}
        </div>
...
export default ContraVoucherReportDetail;
